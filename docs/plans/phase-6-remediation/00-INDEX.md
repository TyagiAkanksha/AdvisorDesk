...
| CHAT RESTORE (post-review, 2026-09-06) | `task-14-openai-provider-swap.md` | both NVIDIA models EOL 410 | ✅ RED 01f2c08 + impl 552592f; opus review Spec+Quality APPROVED-for-merge (0 C, 1 Important boot-guard follow-up→6R-15, 4 doc Minors). Local e2e verified with real OpenAI: chat grounded+cited, refusal, agent tool-calling all work |
| CHAT RESTORE | `task-13-readme-demo.md` (WR-18) | WR-18 | ✅ 75ae326 — README `## Demo` + scripts/demo.sh, both user stories + both run paths, live-verified (demo.sh exit 0 vs prod, cites correct article). Closes PRD §10 DoD. Honest finding: refusals carry non-empty citations under OpenAI embeddings (borderline chunks clear 0.35) → threshold follow-up below |
| FOLLOW-UP (ledgered) | SIMILARITY_THRESHOLD re-calibration | — | 0.35 was tuned for NVIDIA nv-embedqa; OpenAI text-embedding-3-small has a different cosine-sim distribution → borderline chunks clear it, spurious citations on refusals + weaker retrieval_found signal. Re-tune via phase-7 groundedness harness (eval_questions.yaml). Refusal still works (LLM-judged); not urgent |
| CHAT RESTORE | `task-15-provider-aware-boot-guard.md` | 6R-14 review Important + 4 Minors | GATES PROD DEPLOY — briefed, dispatched |
| PROD REDEPLOY | ops (controller+owner, after 6R-15) | — | build+push api image, OPENAI key→SSM, compose env→OpenAI, NON-DESTRUCTIVE re-embed Supabase, restart, verify live |
