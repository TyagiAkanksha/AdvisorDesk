"""Shared Google-OAuth test seam: a fake client + a one-call login shortcut.
...
one fake client, one login helper.
"""
...
def login_as(
    ...
    *,
    ...
    back from `client.app.state.oauth_client`), then hits
    `/api/v1/auth/callback` with that code exactly as Google's redirect
    would — landing the signed session cookie `issue_cookie` sets onto
    `client`. Reused by every later admin-route test (task-01 brief,
    Interfaces block) so callers never need to know the cookie's name or
    format.
