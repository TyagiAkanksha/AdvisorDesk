...
def test_run_agent_bounds_a_never_terminating_llm_with_a_graceful_error(
    ...
    # (8 registered tools, PRD §10) — the ceiling stops the EXCHANGE, not the loop's own normal
    # behavior; this bound is orthogonal to `_MAX_TOOL_CALLS`, which never engages here since no
    # tool is ever called.
    assert llm.seen_tool_schema_names[0] == llm.seen_tool_schema_names[-1]
    assert len(llm.seen_tool_schema_names[0]) == 8
