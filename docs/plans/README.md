...
| 8 | [`phase-8-ui-polish/`](phase-8-ui-polish/00-INDEX.md) | UI polish for both apps in three sub-phases, each its own PR: **A** shared foundation (theme type scale + fonts, twin Markdown renderer, primitives, route files), **B** client shell/home/article/chat, **C** admin shell/sign-in/dashboard/table/editor/connected-apps/agent panel + sign-in error redirect — spec [`phase-8-ui-polish/DESIGN.md`](phase-8-ui-polish/DESIGN.md) | mcp-oauth | A+B+C+t24 merged + deployed (PRs #28, #29, #31, #34); prod bda5084 |
| 9 | [`phase-9-eval-data-loop/`](phase-9-eval-data-loop/00-INDEX.md) | Evaluated + self-improving loop for the 2026-09-24 talk: migration 0009 (feedback, eval runs/results, proposals), RAGAS-named metrics + agent trajectory suite + calibrated judge scorecard + failure taxonomy + 3-run stability, weak-query detection → data-gated proposals, a 16-article equity-comp corpus for a fictional firm (Queen City Wealth Planning) with an 80-question golden set — spec [`phase-9-eval-data-loop/DESIGN.md`](phase-9-eval-data-loop/DESIGN.md) | 8, hygiene | in progress on `feat/eval-data-loop` |
| hygiene | [`hygiene-2026-09/`](hygiene-2026-09/00-INDEX.md) | The phase-8 "ride" list: `isInternalHref` twin, `EmptyState` live region, drop `vite-tsconfig-paths`, `AppShell`-outside-`RequireSession`, content-list + editor-hook tidies, discovery-test env isolation, `@/testing` lint guard — nine small three-agent tasks, one PR | 8 | merged + deployed (PR #36); prod 387af39 |

Statuses here and in each INDEX are a snapshot; **git history is authoritative**. As of this
writing, phases 1–8, `mcp-oauth` and the hygiene batch are merged to `main` and deployed.
