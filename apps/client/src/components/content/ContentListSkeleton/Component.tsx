import { Box, Grid, Skeleton } from '@/components/common';
import { LOADING_LABEL } from '@/lib/copy';

const PLACEHOLDER_COUNT = 6;
const TAG_PLACEHOLDER_COUNT = 5;

// phase-8 task-09 (DESIGN.md §B2): what app/loading.tsx shows while the RSC list fetch is in
// flight — matches ContentListScreen's header + responsive card grid so nothing jumps when data
// lands.
//
// p8 final (M-2): a title line alone jumped once the real header's description text and tag
// filter row landed — add placeholders for both so the whole header settles in one paint.
export default function Component() {
  return (
    <Box role="status" aria-label={LOADING_LABEL}>
      <Skeleton variant="text" width="30%" height={44} />
      <Skeleton variant="text" width="60%" />
      <Box sx={{ display: 'flex', gap: 1, mb: 3 }}>
        {Array.from({ length: TAG_PLACEHOLDER_COUNT }, (_, index) => (
          <Skeleton key={index} variant="rounded" width={88} height={24} />
        ))}
      </Box>
      <Grid container spacing={2}>
        {Array.from({ length: PLACEHOLDER_COUNT }, (_, index) => (
          <Grid key={index} size={{ xs: 12, sm: 6, md: 4 }}>
            <Skeleton variant="rounded" height={140} />
          </Grid>
        ))}
      </Grid>
    </Box>
  );
}
