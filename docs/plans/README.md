...
| hygiene | [`hygiene-2026-09/`](hygiene-2026-09/00-INDEX.md) | The phase-8 "ride" list: `isInternalHref` twin, `EmptyState` live region, drop `vite-tsconfig-paths`, `AppShell`-outside-`RequireSession`, content-list + editor-hook tidies, discovery-test env isolation, `@/testing` lint guard — nine small three-agent tasks, one PR | 8 | in progress on `chore/hygiene-ride` |

Statuses here and in each INDEX are a snapshot; **git history is authoritative**. As of this
writing, phases 1–8 and `mcp-oauth` are merged to `main` and deployed; the hygiene batch is
in progress on `chore/hygiene-ride`.
