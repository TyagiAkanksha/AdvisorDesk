"""Regression tests for phase-4 task-04 review round 1, finding I1 (`app/seed.py`).

Task brief: docs/plans/phase-4-rag-assistant/task-04-seed-content-eval-set.md.
Review: .superpowers/sdd/reports/p4-t04-review.md, finding I1 — `seed_all`'s idempotency check
previously keyed on a seed file's filename stem, while `create_draft` (via
`app.services.content.generate_slug`) actually persists a slug derived from the file's frontmatter
`title`. When the two disagree — impossible for the 21 committed `seed/sample_content/*.md` files
(`tests/test_seed.py`'s own DB-backed slug-join test pins title-slugifies-to-filename for all of
them), but not guaranteed for an out-of-repo/user-supplied `content_dir` — every re-run of
`seed_all` would silently re-create the row with a `-2`, `-3`, ... collision-suffixed slug and
re-embed it, forever (review probe B, reproduced independently below).

`app/seed.py`'s fix keys the idempotency check on `title` instead (`_already_seeded`). This file
proves the fix: a throwaway `tmp_path` content directory (never `seed/sample_content/`) holding one
file whose filename stem does NOT match `generate_slug`'s output for its own frontmatter title, run
through `seed_all` twice.
